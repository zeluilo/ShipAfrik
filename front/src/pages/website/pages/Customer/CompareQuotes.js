...
const CompareQuotes = () => {
    ...
    const handleBuyNow = () => {
        console.log('Navigating with shipment and quote data:', { selectedShipment, quotes });
        window.scrollTo(0, 0);
        navigate('/order-summary', { state: { selectedShipment, quotes } });
    };
                                                                                ...
                                                                                <table className="table table-bordered table-sm" style={{ maxWidth: '80%', margin: '0 auto' }}>
                                                                                    ...
                                                                                    <tbody>
                                                                                        {shipment.boxSizes.map(box => {
                                                                                            console.log('Quotes:', quotes); // Check the value of quotes

                                                                                            // Ensure quotes is an object and has boxSizes
                                                                                            const quoteSizes = Array.isArray(quotes) ? quotes.map(q => q.boxSizes).flat() : quotes.boxSizes || [];
                                                                                            console.log('Quote Sizes:', quoteSizes);

                                                                                            // Find the matching box size in quote sizes
                                                                                            const boxInQuote = quoteSizes.find(b => b.size === box.size);
                                                                                            const requestedQuantity = boxInQuote ? boxInQuote.quantity : 0; // Get the requested quantity or default to 0
